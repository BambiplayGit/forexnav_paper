...
std::cout << "in planGlobalPath" << std::endl;
	...
	if (node == nullptr || node->polys_.size() < 2) {
		ROS_ERROR("[Plan manage]: Node is null or has insufficient polyhedrons (size=%zu). Cannot plan global path.", 
				  node ? node->polys_.size() : 0);
		return false;
	}
...
bool DynamicPlanManagerPOP::setLinearConstraintsandVertices(vec_E<Polyhedron3D> &polys_, std::vector<Eigen::Vector3d> &points_for_poly_){
...
bool DynamicPlanManagerPOP::decomposePolys(){
	polys_.clear();
