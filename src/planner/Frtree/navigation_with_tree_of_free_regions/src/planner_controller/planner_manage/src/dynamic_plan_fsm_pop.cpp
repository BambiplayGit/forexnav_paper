...
// std::cout << "fsm param loaded" << endl;
    ...
    cmd_timer_             = node_.createTimer(ros::Duration(0.02), &DynamicReplanFSMPOP::publishCmdCallback, this);  // 50Hz for smooth tracking
...
std::cout << "backtrack flag is true" << endl;
            ...
            if(!planner_manager_->backtrackFlag_){
                ...
                if (success && planner_manager_->current_node_->polys_.size() >= 2) {
...
std::cout << "back track nodes's center list[2]: " << planner_manager_->backtrack_node_->center_list_[2].transpose() << endl;
            ...
            if (success && planner_manager_->backtrack_node_->polys_.size() >= 2) {
                vec_E<Polyhedron3D> first_two_polys;
...
std::cout << "twist cmd: " << twist_cmd.linear.x << " " << twist_cmd.linear.y << " " << twist_cmd.angular.z << endl;
    ...
    // Publish pos_cmd for direct position tracking with interpolation
    geometry_msgs::PoseStamped pos_cmd;
    pos_cmd.header.stamp = ros::Time::now();
    pos_cmd.header.frame_id = "odom";
    
    int target_idx = (index + 1 < traj_from_altro_.size()) ? index + 1 : traj_from_altro_.size() - 1;
    ...
    if (dist > step_size) {
        ...
        // Close enough, use target directly
        interp_pos = target_pos;
        interp_yaw = target_yaw;
    }
    
    pos_cmd.pose.position.x = interp_pos(0);
    pos_cmd.pose.position.y = interp_pos(1);
    pos_cmd.pose.position.z = interp_pos(2);
    Eigen::Quaterniond q_cmd(Eigen::AngleAxisd(interp_yaw, Eigen::Vector3d::UnitZ()));
    pos_cmd.pose.orientation.w = q_cmd.w();
    pos_cmd.pose.orientation.x = q_cmd.x();
    pos_cmd.pose.orientation.y = q_cmd.y();
    pos_cmd.pose.orientation.z = q_cmd.z();
...
std::cout << "in getTrajByALTRO" << endl;
    ...
    // Safety check: ensure node has at least 2 polyhedrons
    if (node == nullptr || node->polys_.size() < 2) {
        ROS_ERROR("[ALTRO]: Node is null or has insufficient polyhedrons. Cannot compute trajectory.");
        return;
    }
