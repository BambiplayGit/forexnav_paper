...
void GridMap::initMap(ros::NodeHandle &nh)
{
    ...
    // TF initialization - MUST be done before timers start
    ...
    ROS_INFO("[Local map]: Waiting for TF %s->%s ...", mp_.base_id_.c_str(), mp_.scan_sensor_frame_id_.c_str());
    try
    {
        *(mp_.TF_base_to_scan_ptr_) = tfBuffer_scan.lookupTransform(mp_.base_id_, mp_.scan_sensor_frame_id_, ros::Time(0), ros::Duration(5.0));
        ROS_INFO("[Local map]: Got TF %s->%s", mp_.base_id_.c_str(), mp_.scan_sensor_frame_id_.c_str());
    }
    catch (tf2::TransformException &ex)
    {
        ROS_ERROR("fail to listen the transform of %s->%s. Error message: %s", mp_.base_id_.c_str(), mp_.scan_sensor_frame_id_.c_str(), ex.what());
        return;
    }

    ROS_INFO("[Local map]: Waiting for TF %s->%s ...", mp_.base_id_.c_str(), mp_.pointcloud_sensor_frame_id_.c_str());
    try
    {
        *(mp_.TF_base_to_sensor_ptr_) = tfBuffer_pc.lookupTransform(mp_.base_id_, mp_.pointcloud_sensor_frame_id_, ros::Time(0), ros::Duration(5.0));
        ROS_INFO("[Local map]: Got TF %s->%s", mp_.base_id_.c_str(), mp_.pointcloud_sensor_frame_id_.c_str());
    }
    catch (tf2::TransformException &ex)
    {
        ROS_ERROR("fail to listen the transform of %s->%s. Error message: %s", mp_.base_id_.c_str(), mp_.pointcloud_sensor_frame_id_.c_str(), ex.what());
        return;
    }

    // sensor sub - AFTER TF is ready
    ...
    // timer callbacks - MUST be created AFTER TF lookup succeeds
    occ_timer_ = public_nh.createTimer(ros::Duration(0.05), &GridMap::updateOccupancyCallback, this);
    vis_timer_ = public_nh.createTimer(ros::Duration(0.05), &GridMap::visCallback, this);

    ROS_INFO("[Local map]: Initialization complete!");
}
...
void GridMap::slicePointCloudInxyPlane(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> &point_cloud)
{
    ...
    // std::cout << "[Local map] size of point cloud before filter: " << point_cloud->points.size() << std::endl;
    ...
    pass.filter(*point_cloud);
    // std::cout << "[Local map] after y filter: " << point_cloud->points.size() << std::endl;
}
