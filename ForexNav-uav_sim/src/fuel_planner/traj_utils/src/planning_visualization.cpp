...
void PlanningVisualization::drawFrontierGraph(const Eigen::Vector3d& start,
                                              const Eigen::Vector3d& goal,
                                              const std::vector<Eigen::Vector3d>& frontier_nodes,
                                              const std::string& ns, const int pub_id) {
  // 1. Draw frontier graph nodes (larger purple spheres)
  drawSpheres(frontier_nodes, 0.25, Eigen::Vector4d(1.0, 0.0, 1.0, 1.0), ns, 0, pub_id);

  // 2. Draw start (green) and goal (red) - larger
  drawSpheres({start}, 0.3, Eigen::Vector4d(0.0, 1.0, 0.0, 1.0), ns, 1, pub_id);  // green
  drawSpheres({goal},  0.3, Eigen::Vector4d(1.0, 0.0, 0.0, 1.0), ns, 2, pub_id);  // red

  // 3. Draw gray lines connecting start & goal to frontiers
  std::vector<Eigen::Vector3d> starts, goals;
  for (const auto& node : frontier_nodes) {
    starts.push_back(start);
    starts.push_back(goal);
    goals.push_back(node);
    goals.push_back(node);
  }
  drawLines(starts, goals, 0.03, Eigen::Vector4d(0.5, 0.5, 0.5, 1.0), ns, 3, pub_id);  // gray
}
void PlanningVisualization::drawFuzzyAstarPaths(
    const std::vector<Eigen::Vector3d>& paths_world,
    const std::string& ns,
    const double& scale,
  ...
  visualization_msgs::Marker mk;
  fillBasicInfo(mk, Eigen::Vector3d(scale, scale, scale), color, ns, id,
                visualization_msgs::Marker::CUBE_LIST);  // ✅ 改为小立方体
