...
public:
  ...
  void drawFrontierGraph(const Eigen::Vector3d& start, const Eigen::Vector3d& goal,
                       const std::vector<Eigen::Vector3d>& frontier_nodes,
                       const std::string& ns = "frontier_graph", const int pub_id = 4);
// 可视化 fuzzy A* 搜索出的多条路径（世界坐标系）
void drawFuzzyAstarPaths(const std::vector<Eigen::Vector3d>& paths_world,
                         const std::string& ns = "fuzzy_astar_path",
                         const double& scale = 0.08,
                         const Eigen::Vector4d& color = Eigen::Vector4d(0.2, 0.7, 1.0, 1.0),
                         const int& id = 9527,
                         const int& pub_id = 0);
