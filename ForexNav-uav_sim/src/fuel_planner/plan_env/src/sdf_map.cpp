...
const nav_msgs::OccupancyGrid& SDFMap::getPredictedMap() const {
  static nav_msgs::OccupancyGrid empty_map; // 防止返回悬空引用
  if (mr_) {
    return mr_->latest_predicted_map_;
  } else {
    ROS_WARN_THROTTLE(1.0, "[SDFMap] MapROS not initialized, returning empty predicted map.");
    return empty_map;
  }
}
