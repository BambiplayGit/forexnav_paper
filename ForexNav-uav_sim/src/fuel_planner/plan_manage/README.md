...
- __path_searching__: Front-end path searching algorithms. Currently it includes a kinodynamic version of A* algorithm that respects the dynamics of quadrotors. The standard A* is also available. **NEW**: Support for Ackermann vehicle kinodynamic planning is now configurable (see Section 4.1).
...
### 4.1 Frontend Planner Selection (NEW)

The system now supports two types of frontend planners:

#### Standard Kinodynamic A* (Default)
- **For**: Drones, quadrotors, holonomic robots
- **Features**: 6DOF motion, full-directional movement
- **State Space**: (x, y, z, vx, vy, vz)

#### Ackermann Kinodynamic A* 
- **For**: Car-like vehicles with Ackermann steering
- **Features**: Non-holonomic constraints, minimum turning radius, forward/backward motion
- **State Space**: (x, y, θ)

#### How to Switch

In your launch file (e.g., `kino_algorithm.xml`), set the parameter:

```xml
<!-- 0: Standard Kinodynamic A* (for drones, holonomic robots) -->
<!-- 1: Ackermann Kinodynamic A* (for car-like vehicles) -->
<param name="manager/frontend_planner_type" value="0" type="int"/>
```

For Ackermann vehicles, use `ackermann_algorithm.xml` which includes vehicle-specific parameters:

```xml
<!-- Vehicle Parameters -->
<param name="vehicle/car_width" value="1.9" type="double"/>
<param name="vehicle/car_length" value="4.88" type="double"/>
<param name="vehicle/car_wheelbase" value="2.85" type="double"/>
<param name="vehicle/car_max_steering_angle" value="45.0" type="double"/>
<!-- ... more parameters ... -->
```

**📖 Detailed Documentation**: See [`docs/FRONTEND_PLANNERS_GUIDE.md`](docs/FRONTEND_PLANNERS_GUIDE.md) for:
- Technical comparison of both planners
- Implementation details
- Parameter tuning guide
- Integration instructions for Ackermann planner

**⚠️ Note**: Full Ackermann integration requires adapting CarLike's kino_astar to work with the FUEL planner's environment representation. See the guide for details.
