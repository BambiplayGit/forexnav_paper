...
void KinoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  // Subscribe to RViz 2D Nav Goal for point-to-point planning
  rviz_goal_sub_ = 
      nh.subscribe("/move_base_simple/goal", 1, &KinoReplanFSM::rvizGoalCallback, this);
  ...
  ROS_INFO("[Kino FSM] Initialized. Waiting for RViz 2D Nav Goal on /move_base_simple/goal");
}
...
void KinoReplanFSM::rvizGoalCallback(const geometry_msgs::PoseStampedConstPtr& msg) {
  // RViz 2D Nav Goal callback for point-to-point planning
  ROS_INFO("[Kino FSM] Received RViz 2D Nav Goal: (%.2f, %.2f, %.2f)",
           msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
  
  trigger_ = true;
  
  // Set goal position
  end_pt_ << msg->pose.position.x, 
             msg->pose.position.y, 
             msg->pose.position.z;
  
  // If z is not set (2D Nav Goal), use default height
  if (fabs(end_pt_(2)) < 0.1) {
    end_pt_(2) = 1.0;  // Default flying height: 1m
    ROS_INFO("[Kino FSM] Using default height: 1.0m");
  }
  
  // Set zero terminal velocity
  end_vel_.setZero();
  have_target_ = true;
  ...
  ROS_INFO("[Kino FSM] Target set to: (%.2f, %.2f, %.2f)", end_pt_(0), end_pt_(1), end_pt_(2));
  
  // Trigger planning
  if (exec_state_ == WAIT_TARGET) {
    changeFSMExecState(GEN_NEW_TRAJ, "RVIZ_GOAL");
  } else if (exec_state_ == EXEC_TRAJ) {
    changeFSMExecState(REPLAN_TRAJ, "RVIZ_GOAL");
  }
}
...
void KinoReplanFSM::execFSMCallback(const ros::TimerEvent& e) {
  ...
  switch (exec_state_) {
    ...
    case EXEC_TRAJ: {
      ...
      Eigen::Vector3d pos = info->getPosition(t_cur);  // Use unified interface (supports MINCO)
    ...
    case REPLAN_TRAJ: {
      ...
      start_pt_ = info->getPosition(t_cur);      // Use unified interface (supports MINCO)
      start_vel_ = info->getVelocity(t_cur);
      start_acc_ = info->getAcceleration(t_cur);
