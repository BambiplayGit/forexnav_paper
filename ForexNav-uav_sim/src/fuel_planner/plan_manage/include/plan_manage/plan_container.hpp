...
struct LocalTrajData {
  ...
  // MINCO trajectory storage (for direct use without B-spline conversion)
  Trajectory<7> minco_traj_;
  bool use_minco_traj_ = false;
  
  // Unified query interface: automatically uses MINCO or B-spline
  inline Eigen::Vector3d getPosition(double t) {
    if (use_minco_traj_) {
      return minco_traj_.getPos(t);
    } else {
      return position_traj_.evaluateDeBoorT(t);
    }
  }
  ...
  inline Eigen::Vector3d getVelocity(double t) {
    if (use_minco_traj_) {
      ...
      return velocity_traj_.evaluateDeBoorT(t);
    }
  }
  ...
  inline Eigen::Vector3d getAcceleration(double t) {
    if (use_minco_traj_) {
      ...
      return acceleration_traj_.evaluateDeBoorT(t);
    }
  }
};
