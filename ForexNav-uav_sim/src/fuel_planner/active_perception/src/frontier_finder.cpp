...
FrontierFinder::FrontierFinder(const EDTEnvironment::Ptr& edt, ros::NodeHandle& nh) {
  ...
  // 2D/3D mode parameters
  // 2D mode: frontier search restricted to a fixed height plane (for ground robots/legged robots)
  // 3D mode: frontier search in full 3D space (for aerial robots)
  nh.param("frontier/use_2d_mode", use_2d_mode_, false);
  nh.param("frontier/fixed_height_2d", fixed_height_2d_, 1.0);

  if (use_2d_mode_) {
    ROS_INFO("[FrontierFinder] Using 2D mode with fixed height: %.2f m", fixed_height_2d_);
  } else {
    ROS_INFO("[FrontierFinder] Using 3D mode");
  }
...
void FrontierFinder::searchFrontiers() {
  ...
  if (use_2d_mode_) {
    // 2D mode: only search at fixed height
    Vector3d fixed_height_pos(0, 0, fixed_height_2d_);
    Eigen::Vector3i fixed_z_id;
    edt_env_->sdf_map_->posToIndex(fixed_height_pos, fixed_z_id);
    int z = fixed_z_id(2);
    
    // Clamp z to valid range
    z = std::max(min_id(2), std::min(z, max_id(2)));
    
    for (int x = min_id(0); x <= max_id(0); ++x)
      for (int y = min_id(1); y <= max_id(1); ++y) {
        // Scanning the updated region to find seeds of frontiers at fixed height
    ...
    // 3D mode: search in full 3D space
    for (int x = min_id(0); x <= max_id(0); ++x)
      for (int y = min_id(1); y <= max_id(1); ++y)
...
void FrontierFinder::expandFrontier(
  ...
  while (!cell_queue.empty()) {
    ...
    auto nbrs = use_2d_mode_ ? tenNeighbors(cur) : allNeighbors(cur);
    ...
    for (auto nbr : nbrs) {
      ...
      // Height constraints
      if (use_2d_mode_) {
        // In 2D mode, allow small height deviation but mostly stay at fixed height
        if (fabs(pos[2] - fixed_height_2d_) > resolution_ * 2.0) continue;
      } else {
        // In 3D mode, remove noise close to ground
  ...
  if (expanded.size() > cluster_min_) {
    ...
    // allocate a permanent graph id
    frontier.graph_id_ = global_graph_id_counter_++;
...
void FrontierFinder::updateFrontierCostMatrix() {
  ...
  if (!removed_ids_.empty()) {
    // CRITICAL FIX: Sort removed_ids_ in descending order and delete from back to front
    // This prevents index shifting issues when deleting elements
    // When we delete an element, all subsequent indices shift by -1
    // By deleting from largest index to smallest, we avoid this problem
    std::sort(removed_ids_.begin(), removed_ids_.end(), std::greater<int>());
    
    // Delete path and cost for removed clusters
    for (auto it = frontiers_.begin(); it != first_new_ftr_; ++it) {
      // Safety check: ensure costs_ and paths_ have the same size
      if (it->costs_.size() != it->paths_.size()) {
        ROS_ERROR("[FrontierFinder] Mismatch: costs_.size()=%zu, paths_.size()=%zu, skipping this frontier", 
                  it->costs_.size(), it->paths_.size());
        continue;
      }
      
      // Delete from back to front (largest index to smallest) to avoid index shifting
      for (int removed_idx : removed_ids_) {
        // Validate index before deletion
        if (removed_idx < 0 || removed_idx >= (int)it->costs_.size()) {
          ROS_WARN("[FrontierFinder] Invalid removed_idx=%d, costs_.size()=%zu, skipping", 
                   removed_idx, it->costs_.size());
          continue;
        }
        
        // Move iterator to the position to delete
        auto cost_iter = it->costs_.begin();
        auto path_iter = it->paths_.begin();
        std::advance(cost_iter, removed_idx);
        std::advance(path_iter, removed_idx);
        
        // Erase the element (this invalidates the iterator, but we don't reuse it)
...
void FrontierFinder::computeFrontierInfo(Frontier& ftr) {
  ...
  // In 2D mode, override average height to fixed height
  if (use_2d_mode_) {
    ftr.average_[2] = fixed_height_2d_;
  }
...
void FrontierFinder::getFrontierCenters() {
  frontier_nodes_.clear();
  for (const auto& ftr : frontiers_) {
    frontier_nodes_.push_back(ftr.average_);
  }
}
...
void FrontierFinder::sampleViewpoints(Frontier& frontier) {
  ...
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_;
    ...
    for (double phi = -M_PI; phi < M_PI; phi += candidate_dphi_) {
      ...
      if (use_2d_mode_) {
        // In 2D mode, sample at fixed height
        sample_pos = frontier.average_;
        sample_pos[2] = fixed_height_2d_;  // Override z with fixed height
        sample_pos += rc * Vector3d(cos(phi), sin(phi), 0);
      } else {
        // In 3D mode, sample at frontier's average height
...
inline vector<Eigen::Vector3i> FrontierFinder::tenNeighbors(const Eigen::Vector3i& voxel) {
  ...
  // 8 horizontal neighbors (2D plane)
  for (int x = -1; x <= 1; ++x) {
    for (int y = -1; y <= 1; ++y) {
      if (x == 0 && y == 0) continue;
  ...
  // 2 vertical neighbors (up and down)
  neighbors[count++] = voxel - Eigen::Vector3i(0, 0, 1);
  neighbors[count++] = voxel + Eigen::Vector3i(0, 0, 1);
  return neighbors;
}
