...
struct Frontier {
  ...
  // For ForexNav Frontier Graph
  int graph_id_; // incremental, should not reset
  int graph_parent_id_;
};
...
public:
  ...
  // Get 2D mode status
  inline bool is2DMode() const { return use_2d_mode_; }
  inline double getFixedHeight() const { return fixed_height_2d_; }

  shared_ptr<PerceptionUtils> percep_utils_;
  std::vector<Eigen::Vector3d> frontier_nodes_;
...
private:
  ...
  // 2D/3D mode switch
  bool use_2d_mode_;
  double fixed_height_2d_;  // Fixed height for 2D mode
  ...
  // ForexNav 
  int global_graph_id_counter_{0};
