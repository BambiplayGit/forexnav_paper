...
void PolynomialTraj::waypointsTraj(const Eigen::MatrixXd& positions, const Eigen::Vector3d& start_vel,
  ...
  if (seg_num <= 0) {
    std::cerr << "[PolynomialTraj] Error: seg_num must be > 0, got " << seg_num << std::endl;
    poly_traj.reset();
    return;
  }
  
  // positions should have seg_num + 1 rows (one point per segment endpoint)
  if (positions.rows() < seg_num + 1) {
    std::cerr << "[PolynomialTraj] Error: positions matrix has " << positions.rows() 
              << " rows, but need at least " << (seg_num + 1) << " rows (seg_num=" << seg_num << ")" << std::endl;
    std::cerr << "[PolynomialTraj] positions.rows()=" << positions.rows() 
              << ", positions.cols()=" << positions.cols() << std::endl;
    poly_traj.reset();
    return;
  }
  
  if (positions.cols() < 3) {
    std::cerr << "[PolynomialTraj] Error: positions matrix must have at least 3 columns, got " 
              << positions.cols() << std::endl;
    poly_traj.reset();
    return;
  }
  
  // Additional validation: check times vector
  for (int i = 0; i < seg_num; i++) {
    if (times(i) <= 0) {
      std::cerr << "[PolynomialTraj] Error: times(" << i << ") must be > 0, got " << times(i) << std::endl;
      poly_traj.reset();
      return;
    }
  }
  
  // Special case: seg_num = 1 requires special handling
  // For single segment, we don't need free derivatives (num_p = 0)
  if (seg_num == 1) {
    // Handle single segment case directly - all derivatives are fixed
    Eigen::MatrixXd Ab = Eigen::MatrixXd::Zero(6, 6);
    for (int i = 0; i < 3; i++) {
      int fact_i = 1;
      for (int k = i; k > 0; k--) fact_i *= k;
      Ab(2 * i, i) = fact_i;
      
      for (int j = i; j < 6; j++) {
        int fact_j = 1, fact_j_i = 1;
        for (int k = j; k > 0; k--) fact_j *= k;
        for (int k = j - i; k > 0; k--) fact_j_i *= k;
        if (j - i == 0) fact_j_i = 1;
        Ab(2 * i + 1, j) = (fact_j / fact_j_i) * pow(times(0), j - i);
      }
    }
    ...
    Eigen::VectorXd Px = Ab.inverse() * D;
    ...
    D(4) = start_acc(1);
    D(5) = end_acc(1);
    Eigen::VectorXd Py = Ab.inverse() * D;
    ...
    D(4) = start_acc(2);
    D(5) = end_acc(2);
    Eigen::VectorXd Pz = Ab.inverse() * D;
    
    poly_traj.reset();
    Polynomial poly(Px, Py, Pz, times(0));
